...
From the above output we can see that 140 homes have no central air with only 5 of them being bonus eligible. However, there are 1909 homes that have central air with 851 of them being bonus eligible. For an even more detailed breakdown we can use the ```CrossTable``` function.
...
The advantage of the ```CrossTable``` function is that we can easily get not only the frequencies, but the cell, row, and column proportions. For example, the third number in each cell gives us the row proportion. For homes without central air, 96.4% of them are not bonus eligible, while 3.6% of them are. For homes with central air, 55.4% of the homes are not bonus eligible, while 44.6% of them are. This would appear that the distribution of bonus eligible homes changes across levels of central air - a relationship between the two variables. This expected relationship needs to be tested statistically for verification.
...
Two common $\chi^2$ tests are the Pearson and Likelihood Ratio $\chi^2$ tests. They compare the observed count of observations in each cell of a cross-tabulation table between two variables to their expected count **if** there was no relationship. The expected cell count applies the overall distribution of one variable across all the levels of the other variable. For example, overall 58% of all homes are not bonus eligible. **If** that were to apply to every level of central air, then the 140 homes without central air would be expected to have 81.2 ($ = 140 \times 0.58$) of them would be bonus eligible while 58.8 ($ = 140 \times 0.42$) of them would not be bonus eligible. We actually observe 135 and 5 homes for each of these categories respectively. The further the observed data is from the expected data, the more evidence we have that there is a relationship between the two variables.
...
Let's look at the row without central air. The probability that a home without central air is not bonus eligible is 96.4%. That implies that the odds of not being bonus eligible in homes without central air is 26.78 (= 0.964/0.036). For homes with central air, the odds of not being bonus eligible are 1.24 (= 0.554/0.446). The odds ratio between these two would be approximately 21.7 (= 26.7778/1.2422). In other words, homes without central air are 21.7 times more likely (in terms of odds) to not be bonus eligible as compared to homes with central air. This relationship is intuitive based on the numbers we have seen. Without going into details, it can also be shown that homes with central air are 21.7 times as likely (in terms of odds) to be bonus eligible.
...
```{r label='logistic', fig.align='center', fig.cap = 'Example of a Bimodal Distribution', echo=F, out.width="50%"}
...
As with linear regression in Chapters \@ref(sel) and \@ref(mlr), logistic regression uses the same approaches to doing variable selection. In fact, the same function are used as well. Let's use the ```step``` function to apply a forward and backward selection to the logistic regression model.
