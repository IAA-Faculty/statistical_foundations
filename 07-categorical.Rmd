...
From the above output we can see that 133 homes have no central air with only 2 of them being bonus eligible. However, there are 1918 homes that have central air with 849 of them being bonus eligible. For an even more detailed breakdown we can use the ```CrossTable``` function.
...
The advantage of the ```CrossTable``` function is that we can easily get not only the frequencies, but the cell, row, and column proportions. For example, the third number in each cell gives us the row proportion. For homes without central air, 98.5% of them are not bonus eligible, while 1.5% of them are. For homes with central air, 55.7% of the homes are not bonus eligible, while 44.3% of them are. This would appear that the distribution of bonus eligible homes changes across levels of central air - a relationship between the two variables. This expected relationship needs to be tested statistically for verification.
...
```{python}
counts = pd.crosstab(train['Central_Air'], train['Bonus'], margins=True)

# Get row proportions 
row_pct = pd.crosstab(train['Central_Air'], train['Bonus'], normalize='index') 

# Get column proportions 
col_pct = pd.crosstab(train['Central_Air'], train['Bonus'], normalize='columns') 

# Get table proportions
all_pct = pd.crosstab(train['Central_Air'], train['Bonus'], normalize='all') 
...
Two common $\chi^2$ tests are the Pearson and Likelihood Ratio $\chi^2$ tests. They compare the observed count of observations in each cell of a cross-tabulation table between two variables to their expected count **if** there was no relationship. The expected cell count applies the overall distribution of one variable across all the levels of the other variable. For example, overall 58.5% of all homes are not bonus eligible. **If** that were to apply to every level of central air, then the 133 homes without central air would be expected to have 77.8 ( $ = 133 \times 0.585 $ ) of them would be bonus eligible while 55.2 ( $ = 133 \times 0.415$ ) of them would not be bonus eligible. We actually observe 131 and 2 homes for each of these categories respectively. The further the observed data is from the expected data, the more evidence we have that there is a relationship between the two variables.
...
Let's look at the row without central air. The probability that a home without central air is not bonus eligible is 98.5%. That implies that the odds of not being bonus eligible in homes without central air is 65.7 (= 0.985/0.015). For homes with central air, the odds of not being bonus eligible are 1.26 (= 0.557/0.443). The odds ratio between these two would be approximately 52.14 (= 65.7/1.26). In other words, homes without central air are 52.14 times more likely (in terms of odds) to not be bonus eligible as compared to homes with central air. This relationship is intuitive based on the numbers we have seen. Without going into details, it can also be shown that homes with central air are 52.14 times as likely (in terms of odds) to be bonus eligible.
...
```{python}
table = pd.crosstab(train['Central_Air'], train['Bonus'])

# Convert to a statsmodels 2x2 Table object
sm_table = sma.stats.Table2x2(table.values)

print(f"Odds Ratio: {sm_table.oddsratio:.4f}")
print(f"Log Odds Ratio: {sm_table.log_oddsratio:.4f}")
print(f"Odds Ratio 95% CI: {sm_table.oddsratio_confint()}")
```
...
lp_model = smf.ols("Bonus ~ Gr_Liv_Area", data = train).fit()
pred_probabilities = lp_model.predict(train)
concordance = roc_auc_score(train['Bonus'], pred_probabilities)

print(f"\nConcordance Statistic (C-Statistic): {concordance:.4f}")
