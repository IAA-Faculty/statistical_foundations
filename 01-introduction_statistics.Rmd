...
We can gather based on the output that our 95% confidence interval for the mean of `Sale_Price` is [177902.3, 183689.9]. This function also outputs some extra information that relates to hypothesis testing which we will discuss in Section \@ref(hypotest). For now, if we'd like to only pull the output containing the confidence interval information, we can specify `$conf.int` to our code:
