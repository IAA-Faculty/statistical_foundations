...
The null and alternative hypotheses for this test are defined as follows:
$$H_0: \text{ the medians of the two groups are equal }  $$
$$H_a: \text{ the medians of the two groups are NOT equal }   $$
...
### Another example (A/B testing)

In marketing, A/B testing is a common strategy.  Whether they are testing to see if a new design increases the click rate or if a new promotion increases revenue.  The following data set (ab_test_data.csv) is a simulated data set. Let's suppose that a company wants to see if customers exposed to the promotion have significantly higher revenue.  Some customers were exposed to the new promotion (Treatment) and others were not (Control). 


#### Rcode

```{r}
ab_test<- read.csv("https://raw.githubusercontent.com/IAA-Faculty/statistical_foundations/master/ab_test_data.csv")

summary_stats <- ab_test %>%
  group_by(group) %>%
  summarise(
    n           = n(),
    avg_revenue = mean(revenue) # includes $0 for non-clickers
  )
print(summary_stats)

control   <- ab_test %>% filter(group == "control")
treatment <- ab_test %>% filter(group == "treatment")

ggplot(data = ab_test, aes(sample = revenue, color = group)) +
        stat_qq() +
        stat_qq_line()

leveneTest(revenue~factor(group),data=ab_test)

t_result <- t.test(treatment$revenue, control$revenue, alternative = "greater")
print(t_result)

cat("\n--- Revenue Results ---\n")
cat(sprintf("Control avg revenue/user:   $%.2f\n", mean(control$revenue)))
cat(sprintf("Treatment avg revenue/user: $%.2f\n", mean(treatment$revenue)))
...
```{python}
ab_test = pd.read_csv("https://raw.githubusercontent.com/IAA-Faculty/statistical_foundations/master/ab_test_data.csv")

p = (
    ggplot(ab_test, aes(sample='revenue', color='group')) +
    stat_qq() +
    stat_qq_line()
)
...
sp.stats.levene(ab_test[ab_test["group"] == 'control']['revenue'], ab_test[ab_test["group"] == 'treatment']['revenue'])

sp.stats.ttest_ind(ab_test[ab_test["group"] == 'treatment']['revenue'], ab_test[ab_test["group"] == 'control']['revenue'], equal_var = False, alternative='greater')
```
