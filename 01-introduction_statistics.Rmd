...
Getting summary information for Sales Price:

```{python}
r.ames["Sale_Price"].describe()
```

Getting summary statistics by External Quality:

```{python}
print("Mean:")
r.ames.groupby(["Exter_Qual"])["Sale_Price"].mean()
print("Standard Deviation:")
r.ames.groupby(["Exter_Qual"])["Sale_Price"].std()
print("Minimum:")
r.ames.groupby(["Exter_Qual"])["Sale_Price"].min()
print("Maximum:")
r.ames.groupby(["Exter_Qual"])["Sale_Price"].max()
```
...
Showing overlay of histograms for Sales Price by Central Air:

```{python}


sns.histplot(data=r.ames, x="Sale_Price", hue="Central_Air")
```

Same histogram, but now as a relative frequency histogram:

```{python}


sns.histplot(data=r.ames, x="Sale_Price", hue="Central_Air",stat="probability")
```

Overlay of density plots (for Sales Price broken down by Central Air):

```{python}
sns.displot(r.ames, x="Sale_Price", hue="Central_Air", kind="kde", fill=True)
```
...
```{python}
sns.boxplot( y=r.ames["Sale_Price"], x=r.ames["Central_Air"] )
plt.show()
```
...
```{python}
x=r.ames["Sale_Price"]
stats.t.interval(alpha=0.95, df=len(x)-1, loc=np.mean(x), scale=stats.sem(x)) 
```
...
nobs=100
value=0.5
statsmodels.stats.proportion.proportions_ztest(count,nobs,value,alternative='two-sided',prop_var=0.5)
...
```{python}
x=r.ames["Sale_Price"]
stats.ttest_1samp(x, popmean=178000,alternative='two-sided')
```
...
```{python}
x=r.ames["Sale_Price"]
stats.ttest_1samp(x, popmean=178000,alternative='greater')
```
...
sm.qqplot(yes["Sale_Price"], line='45',fit=True)
...
sm.qqplot(no["Sale_Price"], line='45',fit=True)
```
...
yes=r.ames[r.ames["Central_Air"]=="Y"]
no=r.ames[r.ames["Central_Air"]=="N"]
stats.ttest_ind(yes["Sale_Price"], no["Sale_Price"], equal_var=False,alternative="two-sided")
```
...
```{python}
stats.mannwhitneyu(yes["Sale_Price"], no["Sale_Price"],use_continuity=True, alternative='two-sided')
```
