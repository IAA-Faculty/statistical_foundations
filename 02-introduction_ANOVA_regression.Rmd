...
```{python}
ax = sns.displot(data = ames_py, x = ames_py['Sale_Price']/1000, hue = "Exter_Qual")
...
```{python}
ax = sns.displot(data = ames_py, x = ames_py['Sale_Price']/1000, hue = "Exter_Qual", common_norm = False, kind = "kde", fill = True)
ax.set(xlabel = 'Sales Price (Thousands $)',
       ylabel = 'Density')
plt.show()
```
...
```{python}
ax = sns.catplot(data = ames_py, x = "Exter_Qual", y = "Sale_Price", kind = "box")
plt.show()
```
...
```{python}
ax = sns.pairplot(data = train[['Year_Built', 'Total_Bsmt_SF', 'First_Flr_SF', 'Gr_Liv_Area', 'Sale_Price']])
plt.show()
```
