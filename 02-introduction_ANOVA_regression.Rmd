...
\begin{equation}
\mathbf{y} = \beta_0 + \beta_1\mathbf{x_1} + \dots + \beta_k\mathbf{x_k} + \boldsymbol \epsilon 
(\#eq:linmod)
\end{equation}
...
\begin{equation}
y=\beta_0 + \beta_ax_a+\beta_bx_b+\beta_cx_c + \epsilon (\#eq:anova1) 
\end{equation}
...
<li> $\epsilon$ is called the __residual__. It is the error between the prediction and the actual response value.  
</ul>

A _one-way_ ANOVA model only contains a single input variable of interest. Equation \@ref(eq:anova1), while it has 3 dummy variable inputs, only contains a single nominal attribute. In \@ref(chapter3), we will add more inputs to the equation via two-way ANOVA and multivariate regression models. 
...
To use Dunnett's test, we must install the `DescTools` package. The control group to which all other groups will be compared is designated by the `control=` option. 
...
It is interesting to note that Equation \@ref(eq:veccorrelation) is identical to the formula for the cosine of the angle between to vectors. While this geometrical relationship does not benefit our intuition^[The n-dimensional vectors \mathbf{x_c} and \mathbf{y_c} live in the vast _sample space_ where a single point/vector is one possible sample of n objects; this space can be difficult to grasp mentally], it is noteworthy nonetheless. 
...
(\#eq:slr)
\end{equation}
where $\beta_0$, the __intercept__, gives the expected value of $y$ when $x=0$ and $\beta_1$, the __slope__ gives the expected change in $y$ for a one-unit increase in $x$. The __residual__, $\epsilon$ is the error between our predicted value, $\hat{y}=\beta_0 + \beta_1x$ and our actual value $y$. 

As discussed in Section \@ref(evp), a simple linear regression serves two purposes:
<ol>
...
<li> The random errors have equal variance (heteroskedasticity).
</ol>
