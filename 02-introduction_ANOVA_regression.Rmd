...
Yes, we do have to run the model first to get the residuals, but we should NOT look at the output of the model until we are comforable that the assumptions hold.  Since Normality is fine here, we move on to testing the variances are equal using the Levene test.

```{r}
library(car)
library(stats)

leveneTest(MPG~Country,data=cars2) # Most popular, but depends on Normality
...
And in both cases, we're forced to reject the null hypothesis of equal variances. The Welch's ANOVA is a better alternative when the variances are not equal (still assumes normality).
