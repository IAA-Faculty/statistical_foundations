...
## Two-Way ANOVA
Section \@ref(oneway) details the One-Way ANOVA model using a single categorical predictor variable with $k$ levels to predict our continuous target variable. Now we will generalize this model to include $n$ categorical variables that each have different numbers of levels ($k_1, k_2, ..., k_n$).
...
### Model
We are going to do this with the same approach as in the One-Way ANOVA of Section \@ref(oneway), just with more variables as shown in the following equation:
...
For equal variances, we can still apply the Levene Test for equal variances using the same ```leveneTest``` function as in Section \@ref(oneway).
